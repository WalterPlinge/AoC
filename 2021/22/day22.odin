...
mem_tracked_main :: proc() {
	...
	for step_data in steps_data {
		fields := strings.fields(step_data); defer delete(fields)
		if len(fields) != 2 do continue

		step : Step; defer append(&steps, step)

		if fields[0] == "on" do step.on = true

		coords := strings.split(fields[1], ","); defer delete(coords)
...
QUESTION_1 :: "Execute the reboot steps. Afterward, considering only cubes in the region x=-50..50,y=-50..50,z=-50..50, how many cubes are on?"
QUESTION_2 :: "Starting again with all cubes off, execute all reboot steps. Afterward, considering all cubes, how many cubes are on?"
