...
mem_tracked_main :: proc() {
	...
	defer for s in &list do delete(s)
	...
	add :: proc(a, b : Snailfish) -> Snailfish {
		out := make(Snailfish, len(a) + len(b))
	...
	reduce :: proc(s : ^Snailfish) -> bool {
		index := 0
		for current, i in s do if current.level > 4 {
			prev := i - 1
			next := i + 1

			lhs, rhs := current.value, s[next].value
		...
		for current, i in s do if current.value > 9 {
			first   := current.value / 2
			second  := current.value - first
	...
	magnitude :: proc(s : Snailfish) -> int {
		work := slice.to_dynamic(s[:]); defer delete(work)
		for len(work) > 1 {
			for current, i in &work {
				prev := i - 1
				next := i + 1

				if current.level == work[next].level {
	...
	sum := slice.to_dynamic(list[0][:]); defer delete(sum)
	for reduce(&sum) {}
	for s in 1 ..< len(list) {
		tmp := add(sum, list[s])
		delete(sum)
	...
	largest_magnitude := min(int)
	for a in 0 ..< len(list) {
		for b in 0 ..< len(list) {
			if a == b do continue
			sum := add(list[a], list[b]); defer delete(sum)
			for reduce(&sum) {}
			mag := magnitude(sum)
	...
	fmt.println("\t\ta)", final_magnitude)

	// Part 2
	fmt.println("\t2)", QUESTION_2)
	fmt.println("\t\ta)", largest_magnitude)
}

DAY :: 18

QUESTION_1 :: "What is the magnitude of the final sum?"
QUESTION_2 :: "What is the largest magnitude of any sum of two different snailfish numbers from the homework assignment?"
