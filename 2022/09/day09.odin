...
puzzle :: proc() {
	chain: [10][2]int
	cells: [10]map[[2]int]struct{}; defer for c in cells do delete(c)
	for c in &cells do c[{0, 0}] = {}

	iter := string(PROBLEM_DATA)
	loop: for line in strings.split_lines_iterator(&iter) {
		@static dirs := [][2]int{
			'D' = { 0,-1},
			'L' = {-1, 0},
			'R' = { 1, 0},
		...
		dir := dirs[line[0]]
		steps := strconv.atoi(line[2:])
		for step in 0 ..< steps {
			chain[0] += dir
			cells[0][chain[0]] = {}
			for k in 1 ..< len(chain) {
				head := chain[k - 1]
				knot := &chain[k]
				diff := head - knot^
	...
	ANSWER_1 = len(cells[1])
	ANSWER_2 = len(cells[9])
}
