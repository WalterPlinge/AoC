...
puzzle :: proc() {
	...
	draw :: proc(crt: ^[6][40]bool, cycle, value: int) {
		x := (cycle - 1) % 40
		y := (cycle - (x + 1)) / 40
	...
	for line in strings.split_lines_iterator(&iter) {
		code := strings.fields(line, context.temp_allocator)
		if code[0] == "noop" {
			cycles += 1
			if cycles <= 220 && cycles % 40 == 20 {
				ANSWER_1 += cycles * register
			}
			draw(&crt, cycles, register)
		} else if code[0] == "addx" {
			cycles += 1
			if cycles <= 220 && cycles % 40 == 20 {
				ANSWER_1 += cycles * register
			...
			draw(&crt, cycles, register)
			register += strconv.atoi(code[1])
		}
	}
	sb: strings.Builder
	strings.builder_init_len_cap(&sb, 0, 41 * 6, context.temp_allocator)
	for y in 0 ..< 6 {
		for x in 0 ..< 40 {
			fmt.sbprint(&sb, "#" if crt[y][x] else " ")
		}
		fmt.sbprintln(&sb, "")
	}
